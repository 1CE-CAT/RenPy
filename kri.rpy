...
screen krix:
    ...
    for z in range(1,7):
        for i in range(0,9):
            imagebutton idle("k"+str(globals()["cl"+str(z)][i])+".w")xpos(i*80+520)ypos(z*80+200)action[SetVariable('krix',i),SetVariable('kriy',z),ui.callsinnewcontext("krich")]
#строчка где прописаны названия файлов для спрайтов иконок игры
label krich:
    if krix>=0:
